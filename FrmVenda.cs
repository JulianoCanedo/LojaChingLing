...
namespace LojaCL
{
    public partial class FrmVenda : Form
    {
        ...
        //SqlConnection con = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Programas\\LojaChingLing-master\\DbLoja.mdf;Integrated Security=True;Connect Timeout=30");
        public FrmVenda()
        {
            InitializeComponent();
        }
        ...
        private void cbxProduto_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SqlCommand comd = new SqlCommand("LocalizarProduto", con);
            comd.Parameters.AddWithValue("@Id", cbxProduto.SelectedValue);
        ...
        private void txtQuantidade_Leave(object sender, EventArgs e)
        {
            ...
            clConexao.obterConexao();
            SqlCommand cmdo = new SqlCommand("LocalizarProduto", con);
            cmdo.CommandType = CommandType.StoredProcedure;
            cmdo.Parameters.AddWithValue("@Id", cbxProduto.SelectedValue);
            SqlDataReader rde = cmdo.ExecuteReader();
            int valor1 = 0;
            bool conversaoSucedida = int.TryParse(txtQuantidade.Text, out valor1);
            if (rde.Read())
            {
                int valor2 = Convert.ToInt32(rde["quantidade"].ToString());
                if (valor1 > valor2)
                {
